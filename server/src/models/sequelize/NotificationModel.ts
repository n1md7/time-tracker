...
export default class NotificationModel extends BaseModelSequelize<typeof model> {
    ...
    public async getNotificationsByUserId(userId: number): Promise<NotificationType_[]> {
    ...
    public async updateStatusById(id: number, status: NotificationStatus, userId: number): Promise<void> {
    ...
    public async removeNotificationById(id: number, userId: number): Promise<void> {
        ...
        if (!destroyed) {
            throw new MysqlUpdateException(`removeNotificationById(${id}, ${userId}) didn't remove the record`);
        }
    }
}
